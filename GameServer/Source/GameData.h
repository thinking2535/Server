...
class CGameData
{
	...
	TRewards _Rewards;
	map<EGameMode, SBattleTypeInfo> _BattleTypeInfos; // key : 팀수(게임모드)
...
public:
	...
	inline const map<EGameMode, SBattleTypeInfo>& GetBattleTypeInfos(void) const { return _BattleTypeInfos; }
	...
	const vector<SMultiMap>& GetMaps(const CEngineGameMode* pGameMode_) const;
	_TMapInfo GetMultiMap(const CEngineGameMode* pGameMode_) const;
	inline size_t GetMultiMapCount(void) const { return _MapMeta.OneOnOneMaps.size(); }
